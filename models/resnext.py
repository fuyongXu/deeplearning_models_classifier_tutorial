...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, stride, cardinality, base_width, expansion):
        ...
        D = cardinality * int(base_width * width_ratio)                 #4*32=128
        ...
        self.conv_conv = nn.Conv2d(
            D, D, kernel_size=3, stride=1, padding=0, bias=False
        )
...
class ResNeXt(nn.Module):
    ...
    def block(self, name, in_channels, out_channels, pool_stride=2):
        ...
        for bottleneck in range(self.block_depth):
            ...
            if bottleneck == 0:
                block.add_module(name_, Bottleneck(in_channels, out_channels, pool_stride,
                                                   self.cardinality, self.base_width, self.expansion))

            else:
                block.add_module(name_, Bottleneck(out_channels, out_channels, 1,
