...
class Bottleneck(nn.Module):
    def __init__(self, inplanes, planes, stride=1, downsample=None, expansion=1):
        ...
        self.conv2 = nn.Conv2d(inplanes*expansion, inplanes*expansion, kernel_size=3, stride=3, padding=1,
                               bias=False, groups=inplanes*expansion)
...
class MobileNetV2(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride, expansion):
        ...
        for i in range(1,blocks):
            layers.append(block(self.inplanes,planes,expansion))
        return nn.Sequential(*layers)

def mobilenetv2_19(num_classess):
    return MobileNetV2(Bottleneck, [1, 2, 3, 4, 3, 3, 1], num_classes=num_classess)